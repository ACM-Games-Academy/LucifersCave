...
public class IsEnemyInView : MonoBehaviour
{
    ...
    public bool isEnemyInView(MannequinBehaviour mannequinBehaviour, out float distance)
    {
        distance = float.MaxValue;
        Vector3 screenPoint = cam.WorldToViewportPoint(mannequinBehaviour.transform.position);
        ...
        float maxDistance = Vector3.Distance(cam.transform.position, mannequinBehaviour.transform.position);

        int layerMask = ~LayerMask.GetMask("Mannequin");

        if (Physics.Raycast(cam.transform.position, direction, out RaycastHit hit, maxDistance, layerMask))
        {
